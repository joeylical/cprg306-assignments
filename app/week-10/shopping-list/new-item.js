...
export default function NewItem({updater}) {
        ...
        <div className="flex justify-between">
          <div className="p-2 mt-1 mb-1 rounded-md bg-white text-white w-36">
            <div className="flex justify-between">
              ...
              <div className="flex">
                <button className={buttonClassName} disabled={quantity>=20} onClick={()=>increment(quantity, setQuantity)} type="button">+</button>
                <button className={buttonClassName} disabled={quantity<=1} onClick={()=>decrement(quantity, setQuantity)} type="button">-</button>
              </div>
            </div>
          </div>
          <select
